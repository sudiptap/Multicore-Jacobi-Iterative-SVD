#ifndef __JACOBI_ASYNC_DELAYED_H_
#define __JACOBI_ASYNC_DELAYED_H_

#include "jacobi_svd.hpp"

class JacobiAsyncDelayed : public SVDecomposer<JacobiAsyncDelayed> {
  private:

  public:

    JacobiAsyncDelayed(gsl_matrix *M, Params &params):
      SVDecomposer("JacobiAsyncDelayed", M, params) {
      }
    ~JacobiAsyncDelayed() {
    }
    ...
    int decompose(ofstream &log) {
      ...
      vector<tuple<size_t, size_t, double, double> > history(sweepmax * N * (N-1)/2);
...
#pragma omp critical
          {
            history[update_count++] = make_tuple(j, k, cosine, sine);
          }
...
cout << "update count = " << update_count << endl;
      {
        gsl_matrix_memcpy(A, Aorig);
        for (size_t i=0; i<update_count; i++) {
          size_t j = get<0>(history[i]);
          size_t k = get<1>(history[i]);
          double cosine = get<2>(history[i]);
          double sine = get<3>(history[i]);
...
#endif // __JACOBI_ASYNC_DELAYED_H_
